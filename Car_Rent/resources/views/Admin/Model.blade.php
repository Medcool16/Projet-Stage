            ...
            <div class="block max-w-4xl p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                <form method="POST" action="{{ route('store') }}">
                    ...
                    <div class="mb-3">
                      ...
                      <input type="text" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light" placeholder="Fuel name" name="model">
                    </div>
                    <label  class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Mark</label>
                    <select name="mark" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        @foreach ($data as $i)
                    ...
                    @endif
                    <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 my-3" style="display: inline-block">Submit</button>
                </form>
            </div>
        </div>
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3">Mark at</th>
                ...
                <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                    <td class="px-6 py-2 ">{{ $j->id }}</td>
                    <td class="px-6 py-2 ">{{ $j->model_name }}</td>
                    @foreach ($data as $k)
                        @if ($j->id_mark == $k->id)
                            <td class="px-6 py-2 ">{{ $k->mark_name }}</td>
                        @endif
                    @endforeach
